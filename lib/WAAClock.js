...
WAAClock.prototype.timeStretch = function(events, ratio) {
  var self = this
    ...
    , futureEvents = events.filter(function(event) {
      return currentTime < event._earliestTime
    })
    , tRef = Math.min.apply(Math, futureEvents.map(function(event) { return event.deadline }))
    , tRefStretched = this._absTime(ratio * this._relTime(tRef))

  events.forEach(function(event) {
    if (event.isRepeated()) event.repeat(event.repeatTime * ratio)

    // If event's deadline is still far enough from current time, we translate
    // it to the stretched space.
    if (currentTime < event._earliestTime)
      event.schedule(tRefStretched + ratio * (event.deadline - tRef))
    ...
    // in the stretched space.
      ...
      var deadline = tRefStretched + ratio * (event.deadline - tRef)
      if (currentTime < event._latestTime)
...
WAAClock.prototype._handleEvent = function(event) {
  if (this.context.currentTime < event._latestTime) {
    event.func(event)
    event.emit('executed')
  } else {
    event.emit('expired')
